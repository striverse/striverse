...
import { ChevronDown, ChevronUp, Lock, Save, ShieldCheck, BarChart3 } from "lucide-react";
...
const defaults: Pkg[] = ["LUNA", "AURORA", "ANDROMEDA", "ORION", "CELESTIA"].map((name, i) => ({
  name,
  usdtAmount: [100, 300, 500, 700, 1000][i],
  stvAmount: [100000, 300000, 500000, 700000, 1000000][i],
  sortOrder: i + 1,
  isActive: true,
  isCollapsed: false,
}));
...
export default function Settings() {
  ...
  async function saveHeroStats() {
    ...
    const d = await r.json();
    setMsg(d.message || "Saved");
    if (d.stats) setHeroStats(d.stats);
  }

  return <main className="space-y-8 text-white">
    <header>
      <p className="text-xs uppercase tracking-[.3em] text-cyan-300/70">Developer Control Center</p>
      <h1 className="mt-2 text-3xl font-black">Striverse Settings</h1>
      <p className="mt-2 text-sm text-slate-400">Central configuration for packages, hero statistics, Partner rewards, wallets and operating rules.</p>
    </header>

    <section className="rounded-3xl border border-cyan-400/15 bg-white/[.035] p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">Homepage Statistics</h2>
          <p className="text-sm text-slate-400">Edit the four statistics displayed in the homepage hero. Changes apply to the public website.</p>
        </div>
        {!developer && <Lock className="text-slate-500" size={18} />}
      </div>
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {([
          ["community", "Community", 0],
          ["raised", "Raised (USDT)", 0],
          ["potentialUsers", "Potential Users", 0],
          ["communityDriven", "Community Driven (%)", 0],
          ...
          <label key={key} className="text-xs text-slate-400">
            ...
            <input
              ...
              type="number"
              min="0"
              max={key === "communityDriven" ? 100 : undefined}
              value={heroStats[key]}
              onChange={(e) => updateHero(key, +e.target.value)}
              className="mt-1 w-full rounded-xl bg-black/30 p-3 text-white"
            />
          </label>
        ))}
      </div>
    ...
    <section className="rounded-3xl border border-cyan-400/15 bg-white/[.035] p-5">
      <div className="flex items-center justify-between gap-3">
        <div><h2 className="text-xl font-bold">Packages</h2><p className="text-sm text-slate-400">Pricing and STV allocation. Referral percentages are managed separately.</p></div>
        {!developer && <Lock className="text-slate-500" size={18} />}
      </div>
      <div className="mt-5 space-y-3">{packages.map((p, i) => <div key={p.name} className="rounded-2xl border border-white/10 bg-black/10 p-4">
        <div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="font-bold">{p.name}</span><span className={`rounded-full px-2 py-1 text-[10px] ${p.isActive ? "bg-emerald-400/10 text-emerald-300" : "bg-red-400/10 text-red-300"}`}>{p.isActive ? "ACTIVE" : "INACTIVE"}</span></div>
          <button onClick={() => updatePkg(i, "isCollapsed", !p.isCollapsed)} className="rounded-lg border border-white/10 p-2">{p.isCollapsed ? <ChevronDown size={16} /> : <ChevronUp size={16} />}</button>
        </div>
        {!p.isCollapsed && <div className="mt-4 grid gap-3 md:grid-cols-4"><label className="text-xs text-slate-400">USDT<input disabled={!developer} type="number" value={p.usdtAmount} onChange={e => updatePkg(i, "usdtAmount", +e.target.value)} className="mt-1 w-full rounded-xl bg-black/30 p-3 text-white" /></label><label className="text-xs text-slate-400">STV<input disabled={!developer} type="number" value={p.stvAmount} onChange={e => updatePkg(i, "stvAmount", +e.target.value)} className="mt-1 w-full rounded-xl bg-black/30 p-3 text-white" /></label><label className="flex items-end gap-2 text-sm"><input disabled={!developer} type="checkbox" checked={p.isActive !== false} onChange={e => updatePkg(i, "isActive", e.target.checked)} /> Active</label><div className="text-xs text-slate-500 self-center">No package-level referral %</div></div>}
      </div>)}</div>
      {developer && <button onClick={savePackages} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-5 py-3 font-bold"><Save size={16} /> Save Packages</button>}
    </section>
